...
class PlayableEmbeds : Plugin() {
    ...
    private val youtubeUrlRe =
        Regex(
            "(?:https?://)?(?:(?:www|m)\\.)?(?:youtu\\.be/|youtube(?:-no***REMOVED***)?\\.com/" +
                    "(?:embed/|v/|watch\\?v=|watch\\?.+&v=|shorts/))((\\w|-){11})" +
                    "(?:(?:\\?|&)(?:star)?t=(\\d+))?(?:\\S+)?"
        )
    private val youtubeClipRe =
        Regex(
            "(?:https?://)?(?:(?:www|m)\\.)?(?:youtu\\.be/|youtube(?:-no***REMOVED***)?\\.com/clip/)" +
                    "((\\w|-){36})(?:(?:\\?|&)(?:star)?t=(\\d+))?(?:\\S+)?"
        )
    ...
    private fun addYoutubeEmbed(layout: ViewGroup, url: String) {
        ...
        webView.run {
            val ytUrl = "https://www.youtube-no***REMOVED***.com/embed/$videoId?start=$timestamp"
            loadData(
                """
                <html>
                    <head>
